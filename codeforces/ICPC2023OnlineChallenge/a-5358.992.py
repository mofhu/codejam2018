...
for f in frames:
    ...
    best_kp = {}  # best_kpp[t] {t: [best_kp@t, p: best_p@t]}
    ...
    for t in range(t0, t0+f[4]):
        ...
        if debug:
            print(f'tbs={tbs}, t={t}')
        if p_limit[t] > 0 and tbs > 0:  # can transfer
            for r in range(R):
            ...
            if tbs >= gt:
                ...
                if debug:
                # print(f't={t}, best_kp is {best_kp[t]}, best_sinr is {best_sinr}')
                    print(f'tbs={tbs}, gt={gt}, best_p={best_p}, real_p={real_p}')
                if real_p < best_p:
                ...
                tbs = 0
                flag_transfer = True
            # set best_kp to output
    if flag_transfer:
        for t in best_kp:
            k = best_kp[t][0]
            output[r + k*R + t*K*R -1][user] = max(best_kp[t][1]-0.000001, best_kp[t][1]*0.999999)
            continue
if debug:
    print(p_limit)
# print(output)  # debug
